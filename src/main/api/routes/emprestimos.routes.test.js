...
describe('Emprestimos Routes', () => {
  ...
  test('Deve retornar 200 e uma mensagem de multa aplicada', async () => {
    const livro = await typeormLivroRepository.save(livroDTO);
    const usuario = await typeormUsuariosRepository.save(usuarioDTO);
  ...
  test('Deve retornar um erro caso o campo obrigatório não tenha sido informado', async () => {
    const { statusCode, body } = await request(app)
    ...
    expect(body.erros.fieldErrors).toEqual({
      data_devolucao: ['Data de devolução é obrigatório'],
    });
  });
